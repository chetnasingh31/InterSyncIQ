...
def build_candidate_analysis_payload(resume_text: str, jd_text: str, jd_name: str, jd_source: str) -> Dict:
    ...
    return {
        ...
        "ats_rating": ats_result["ats_rating"],
    ...
    try:
        ...
        for jd_doc in jd_documents:
            ...
            for resume in extracted_resumes:
                ...
                # Calculate ATS score
                ats_result = calculate_ats_score(resume["text"], jd_doc["text"])
                ...
                jd_candidate_map[resume["name"]] = {
                    ...
                    "ats_score": candidate_result["ats_score"],
                    "ats_rating": candidate_result["ats_rating"]
                }
            ...
            avg_ats = round(sum(r["ats_score"] for r in results) / total_candidates) if results else 0
            ...
            jd_analyses.append({
                ...
                "statistics": {
                    ...
                    "average_ats": avg_ats,
                    "top_ats": max([r["ats_score"] for r in results], default=0)
                }
            })
